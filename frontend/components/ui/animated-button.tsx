...
export function AnimatedButton({
  ...
  const baseClasses = cn(
    ...
    variant === "default" &&
      "bg-white text-black hover:border-primary dark:bg-neutral-900 dark:text-white dark:hover:border-primary",
    variant === "indigo" &&
      "bg-white border-blue-600/20 text-black hover:border-primary dark:bg-neutral-900 dark:border-blue-500/30 dark:text-white",
    variant === "dark" &&
      "bg-neutral-950 border-neutral-800 text-white hover:border-primary",
    variant === "outline" &&
      "bg-transparent border-current text-current hover:border-primary",
    className
  );
...
export const Component = ({ label = "Animation Button" }: { label?: string }) => {
  return (
    <div className={cn("flex flex-col items-center gap-4 p-4 rounded-lg")}>
      ...
      >
        <span className="w-48 h-48 rounded rotate-[-40deg] bg-blue-700 absolute bottom-0 left-0 -translate-x-full ease-out duration-500 transition-all translate-y-full mb-9 ml-9 group-hover:ml-0 group-hover:mb-32 group-hover:translate-x-0" />
